...
export default function AllJobsNativeTable({ jobs }: { jobs: any[] }) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-600 border border-blue-200">
            Menunggu
          </span>
        );
      case 'rejected':
      ...
      <div className="p-4 sm:p-6 border-b border-gray-200 bg-gray-50/30">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <Input
              placeholder="Cari posisi, perusahaan, atau status..."
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              className="pl-9 h-9 bg-white border-gray-200 shadow-sm focus-visible:ring-primary/20"
            />
          </div>
        </div>
      </div>
                  ...
                  <div className="flex flex-col items-center justify-center">
                    {inputValue ? (
                       ...
                       <>
                         ...
                         <p className="text-sm mt-1">Tidak ditemukan lowongan apa pun di database.</p>
                       </>
                    )}
                  </div>
                </td>
