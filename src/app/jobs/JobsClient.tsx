    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="space-y-2.5">
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          ...
          <div className="flex-1 w-full bg-white rounded-2xl border-2 border-slate-200/90 shadow-xs focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10 flex items-center px-4 sm:px-5 h-14 sm:h-16 transition-all">
            ...
            <input
              ...
              className="w-full bg-transparent text-base sm:text-lg font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none py-3"
            />
            {inputValue && (
              <button 
                type="button" 
                onClick={() => { setInputValue(''); setSearchQuery(''); }}
                className="text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100 cursor-pointer shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            )}
          ...
          <div className="flex items-center gap-2 self-end sm:self-center w-full sm:w-auto">
            <button
              ...
              className="h-11 px-4 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs sm:text-sm font-semibold flex items-center justify-center gap-1.5 transition-colors lg:hidden shrink-0 cursor-pointer shadow-2xs"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-5 space-y-5">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                <SlidersHorizontal className="w-4 h-4 text-primary" />
                <h3 className="font-bold text-sm text-slate-900">Filter Lowongan</h3>
              </div>
              {hasActiveFilters && (
                <button
            ...
            {/* Urutan */}
            <div>
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-2 font-mono">Urutkan</label>
            ...
            <div className="border-t border-slate-100 pt-4">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-2 font-mono">Tipe Pekerjaan</label>
            ...
            {/* Pendidikan Minimal */}
            <div className="border-t border-slate-100 pt-4">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-2 font-mono">Pendidikan Minimal</label>
              <div className="flex flex-wrap gap-1.5">
                {eduLevels.map(edu => (
                  <button
                    key={edu}
            ...
            {/* Pengalaman Kerja */}
            <div className="border-t border-slate-100 pt-4">
              <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-2 font-mono">Pengalaman</label>
              <div className="flex flex-col gap-1">
                {expLevels.map(exp => (
                  <button
                    key={exp}
        ...
        <main className="lg:col-span-9 space-y-4">
          ...
          <div className="flex items-center justify-between px-1 text-xs text-slate-600 font-medium">
            <span>Menampilkan <strong className="text-slate-900 font-bold">{filteredJobs.length}</strong> lowongan kerja aktif</span>
            {searchQuery && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {isLoading ? (
              ...
              <div className="col-span-full py-16 px-6 text-center bg-white rounded-2xl border border-slate-200">
                <SearchX className="w-10 h-10 text-slate-300 mx-auto mb-3" />
                <h3 className="text-base font-bold text-slate-900 mb-1">Lowongan Tidak Ditemukan</h3>
          ...
          {!isLoading && visibleCount < filteredJobs.length && (
            <div className="text-center pt-4">
              <button
                ...
                className="px-8 py-3 rounded-2xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-800 font-bold text-xs shadow-xs transition-all cursor-pointer"
              >
      ...
      {isFilterModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/50 backdrop-blur-xs p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-md max-h-[85vh] rounded-t-3xl sm:rounded-3xl p-6 flex flex-col animate-in slide-in-from-bottom duration-200 shadow-2xl">
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <Filter className="w-4 h-4 text-primary" />
                <h3 className="font-bold text-sm text-slate-900">Filter Lowongan</h3>
              </div>
              <button onClick={() => setIsFilterModalOpen(false)} className="p-1 text-slate-400 cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 space-y-4 py-4 text-xs">
              <div>
                <label className="font-bold text-slate-700 block mb-1.5">Tipe Pekerjaan</label>
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1.5">Pendidikan Minimal</label>
                <div className="flex flex-wrap gap-1.5">
                  {eduLevels.map(edu => (
                    <button
                      key={edu}
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1.5">Pengalaman Kerja</label>
                <div className="grid grid-cols-2 gap-2">
                  {expLevels.map(exp => (
                    <button
                      key={exp}
            ...
            <div className="pt-3.5 border-t border-slate-100 flex gap-2.5">
              <button 
                onClick={resetAllFilters}
                className="w-1/3 h-11 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 cursor-pointer hover:bg-slate-50"
              >
                Reset
              </button>
              <button 
