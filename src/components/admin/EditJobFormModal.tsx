...
export default function EditJobFormModal({
  ...
  const [location, setLocation] = useState(job?.location || "Timika, Papua Tengah");
  const [contractType, setContractType] = useState(job?.type || "Full-time");
  const [education, setEducation] = useState(job?.education || "Semua");
  const [experience, setExperience] = useState(job?.experience || "Tanpa Pengalaman");
  const [gender, setGender] = useState(job?.gender || "Pria/Wanita");
  const [ageRange, setAgeRange] = useState(
    job?.ageRange && job?.ageRange !== "Bebas" ? job.ageRange.replace(/\D/g, "") : ""
  );
  ...
  const [requirements, setRequirements] = useState(
    job?.requirements && Array.isArray(job.requirements)
      ? job.requirements.map((r: string) => `<p>${r}</p>`).join("")
      : typeof job?.requirements === "string"
      ? job.requirements
      : ""
  );

  const [email, setEmail] = useState(job?.contacts?.email || job?.company?.email || "");
  const [picName, setPicName] = useState(
    job?.picName || job?.contacts?.picName || job?.company?.picName || ""
  );
  const [whatsapp, setWhatsapp] = useState(job?.contacts?.whatsapp || "");
  const [applicationLink, setApplicationLink] = useState(
    job?.contacts?.applicationLink || ""
  );
  ...
  useEffect(() => {
    if (job) {
      ...
      setLocation(job.location || "Timika, Papua Tengah");
      setContractType(job.type || "Full-time");
      setEducation(job.education || "Semua");
      setExperience(job.experience || "Tanpa Pengalaman");
      setGender(job.gender || "Pria/Wanita");
      setAgeRange(
        job.ageRange && job.ageRange !== "Bebas"
          ? job.ageRange.replace(/\D/g, "")
          : ""
      );
      setDeadline(
        job.deadline ? new Date(job.deadline).toISOString().split("T")[0] : ""
      ...
      setEmail(job.contacts?.email || job.company?.email || "");
      setPicName(
        job.picName || job.contacts?.picName || job.company?.picName || ""
      );
      setWhatsapp(job.contacts?.whatsapp || "");
      setApplicationLink(job.contacts?.applicationLink || "");
      setSalaryMinDisplay(formatRupiah(job.salaryMin));
      setSalaryMaxDisplay(formatRupiah(job.salaryMax));
    }
  }, [job]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error(error);
      setIsSubmitting(false);
      toast.error(error?.message || "Terjadi kesalahan sistem saat memperbarui lowongan.");
    }
  };
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <DialogDescription className="mt-0.5 text-xs text-muted-foreground font-medium">
                  ID: <span className="font-mono text-foreground font-semibold">{job?.id}</span> • Perusahaan: <span className="text-primary font-bold">{job?.company?.name || "Perusahaan"}</span>
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
        ...
        <div className="p-4 sm:p-5 border-t border-border/60 shrink-0 bg-card/80 backdrop-blur-md">
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2.5">
            ...
            >
              ...
              <span>{isSubmitting ? "Menyimpan Perubahan..." : "Simpan Perubahan Lowongan"}</span>
            </button>
          </div>
        </div>
      </DialogContent>
